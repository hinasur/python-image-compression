...
def compress_image(input_image_path, sorted_image_path, index_map_path):
    ...
    # Normalize the index_map to the range [0, 255]
    index_map_normalized = (index_map / np.max(index_map) * 255).astype(np.uint8)
    index_map_image = Image.fromarray(index_map_normalized)
...
def restore_image(sorted_image_path, index_map_path, output_image_path):
    ...
    index_map_normalized = np.array(index_map_image)

    # Restore the original index_map from the normalized one
    index_map = (index_map_normalized * np.max(sorted_image.flatten()) / 255).astype(np.int64)
