...
def map_image_to_hilbert_curve(img_array):
    ...
    for i in range(side_len * side_len):
        coords = hilbert_curve.coordinates_from_index(i)
        mapped_img_array[coords[0], coords[1]] = img_array[i // side_len, i % side_len]
...
def map_image_from_hilbert_curve(mapped_img_array):
    ...
    for i in range(side_len * side_len):
        coords = hilbert_curve.coordinates_from_index(i)
        img_array[i // side_len, i % side_len] = mapped_img_array[coords[0], coords[1]]
